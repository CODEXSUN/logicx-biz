(function () {
	const PAGE_NAME = "loom-dashboard";
	const STYLE_ID = "logicx-loom-dashboard-styles";
	...
	const COMMAND_API_PATH = "loom-command";
	// where Send puts the command: the server keeps it until the device's
	// next poll of COMMAND_API_PATH collects it (see loom_dashboard.py)
	const SET_COMMAND_METHOD = "logicx_biz.logicx_hr.loom_dashboard.set_command";
	...
	const GET_STATE_METHOD = "logicx_biz.logicx_hr.loom_dashboard.get_state";
	const POLL_MS = 4000;
	...
	const TABS = [
		...
		{ key: "logs", title: __("Logs"), api_path: "loom-log", content: "request_content" },
		{
			key: "commands",
			title: __("Commands"),
	...
	frappe.pages[PAGE_NAME].on_page_load = function (wrapper) {
		dashboard = new LoomDashboard(wrapper);
	};
	...
	class LoomDashboard {
		constructor(wrapper) {
			this.page = frappe.ui.make_app_page({
				parent: wrapper,
				title: __("Loom Dashboard"),
				single_column: true,
			});

			inject_styles();

			this.$el = $('<div class="logicx-ld"></div>').appendTo(this.page.main).html(render_scaffold());
		...
		setup_events() {
			this.$el.on("click", ".logicx-ld-tab", (e) => {
				e.preventDefault();
		...
		activate_tab(key) {
			...
			this.$el.find(".logicx-ld-tab").each(function () {
				$(this).toggleClass("is-active", $(this).attr("data-tab") === key);
			});
			this.$el.find(".logicx-ld-tabpane").each(function () {
		...
		render_log(tab, rows) {
			...
			this.$log_body(tab.key).html(`<div class="logicx-ld-log">${cards}</div>`);
		}

		$log_body(key) {
			return this.$el.find(`[data-tab-pane="${key}"] .logicx-ld-card-body.is-log`);
		}

		show_note(key, text, is_error) {
			const css_class = is_error ? "logicx-ld-note is-error" : "logicx-ld-note";
	...
	function render_scaffold() {
		const buttons = TABS.map(
			(tab, i) => `
			<button type="button" class="logicx-ld-tab${i === 0 ? " is-active" : ""}"
				data-tab="${tab.key}">
		...
		const panes = TABS.map(
			(tab, i) => `
			<div class="logicx-ld-tabpane${i === 0 ? "" : " hidden"}" data-tab-pane="${tab.key}">
				${tab.api_path
					? `${tab.composer ? render_composer() : ""}
						<div class="logicx-ld-card-body is-log"></div>`
					: `<div class="logicx-ld-card-body logicx-ld-empty"></div>`
				}
			</div>`
		).join("");

		return `
			<div class="logicx-ld-card logicx-ld-tabcard">
				<div class="logicx-ld-tabnav">
					<div class="logicx-ld-tabnav-tabs">${buttons}</div>
					<a href="#" class="logicx-ld-refresh" data-action="refresh">${__("Refresh")}</a>
				</div>
				${panes}
			</div>`;
	}
	...
	function render_composer() {
		return `
			<div class="logicx-ld-composer">
				<div class="logicx-ld-field" data-field="command"></div>
				<div class="logicx-ld-composer-foot">
					<button type="button" class="btn btn-primary btn-sm logicx-ld-send" data-action="send">
						${__("Send")}
					</button>
					<div class="logicx-ld-pending" data-field="pending">${frappe.utils.escape_html(pending_text(""))}</div>
				</div>
			</div>`;
	}
	...
	function render_log_card(content, creation) {
		...
		const body = text
			? `<pre class="logicx-ld-entry-body">${frappe.utils.escape_html(text)}</pre>`
			: `<div class="logicx-ld-entry-body is-empty">${__("Empty")}</div>`;

		return `
			<article class="logicx-ld-entry">
				<header class="logicx-ld-entry-head">
					<span class="logicx-ld-entry-time">${time}</span>
					<span class="logicx-ld-entry-size">${format_bytes(text)}</span>
				</header>
				${body}
			</article>`;
	}
	...
	const PAGE_STYLES = `
		...
		.logicx-ld-field .frappe-control {
			margin-bottom: 0;
		}

		.logicx-ld-field textarea {
			min-height: 80px;
			font-family: var(--font-stack-mono, monospace);
		}

		.logicx-ld-composer-foot {
			display: flex;
