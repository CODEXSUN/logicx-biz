"""Server side of the Loom Dashboard page (page/loom_dashboard).

The Commands tab queues one command for the device. The page stores it here,
in the site's Redis cache under COMMAND_CACHE_KEY, and the ``loom-command``
...
``loom-command`` (request method GET).
"""

import frappe
from frappe import _

COMMAND_CACHE_KEY = "loom_command"
# a command the device has not collected within this long is dropped rather
# than delivered stale on some much later poll
COMMAND_TTL_SEC = 300

# the roles the page itself is limited to (page/loom_dashboard.json)
PAGE_ROLES = ("System Manager", "TM Admin")

COMMAND_SCRIPT = '''# loom-command: GET. answers the command queued on the Loom Dashboard
# (logicx_biz.logicx_hr.loom_dashboard.set_command) and empties the slot,
# so the same command is never delivered twice. pop_command does both: the
# sandbox has no frappe.cache of its own.
frappe.response["message"] = {
	"command": frappe.call("logicx_biz.logicx_hr.loom_dashboard.pop_command"),
}
'''
...
def set_command(command: str) -> None:
	"""Queue `command` for the device's next poll of ``loom-command``.
...
def pop_command() -> str:
	...
	Called by the ``loom-command`` Server Script on the device's behalf;
