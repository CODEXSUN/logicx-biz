(function () {
	frappe.query_reports["Party Outstanding Detailed"] = {
		...
		formatter: function (value, row, column, data, default_formatter) {
			if (DR_CR_FIELDS.includes(column.fieldname)) {
				return format_dr_cr(value, row, column, data, default_formatter);
			}
	...
	const ROUTE = "Party Outstanding Detailed";
	const HEADER_WRAP_CLASS = "logicx-wrap-headers";
	const HEADER_HEIGHT_INCREASE = "15px";
	const FILTER_ROWS_INCREASE = `${0 * 40}px`;

	// the two signed columns: party_outstanding_detailed.py leaves the sign on
	// them (positive is a debit balance, negative a credit one) so the total row
	// still adds up; the sign becomes a Dr/Cr suffix at render time instead
	const DR_CR_FIELDS = ["opening", "balance"];
	...
	// "1,234.00 Dr" -- the amount drops its sign, the side carries it instead.
	// built by hand rather than around default_formatter's output, which is the
	// same right-aligned div, so the suffix can sit inside it
	function format_dr_cr(value, row, column, data, default_formatter) {
		const amount = flt(value);
		// zero -- and the blanks party_outstanding_detailed.py sends as NULL --
		// has no side to name, so it is left to the plain Currency formatter
		if (!amount) return default_formatter(value, row, column, data);

		// written as literals so the translation extractor still sees Dr and Cr
		const side = amount > 0 ? __("Dr") : __("Cr");
		return (
			'<div class="text-right">' +
			format_currency(Math.abs(amount)) +
			' <span class="text-muted">' + side + "</span>" +
			"</div>"
		);
	}
